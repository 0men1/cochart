// Input validation for the market endpoints. Pure and side-effect free so the
// bounds can be unit-tested without spinning up the server (mirrors the shape of
// feedback/validate.ts: a discriminated { ok } union).
...
// Caps the upstream fan-out. A query is split into blocks of (granularity * 300)
// candles, so bounding the candle count bounds how many exchange requests one
// HTTP call can trigger (12_000 / 300 = 40 blocks). This is the core DoS guard.
